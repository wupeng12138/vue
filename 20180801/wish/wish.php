...
switch($act){
	case 'add':
		...
		$i_sql="INSERT INTO  `zns_wish`.`wish` (`ID` ,`username` ,`content`,`pos`,`time`) VALUES (NULL ,  '".$username."',  '".$content."',  '".$pos."',".time().")";
		mysql_query($i_sql);
	...
	case 'update':
		...
		$d_sql="UPDATE wish SET pos='".$pos."' WHERE id=".$id;
		mysql_query($d_sql);
		echo_str('{error:0, msg:"更新位置成功"}');
		break;
}
?>